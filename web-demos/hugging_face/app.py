...
os.chdir(ROOT)  # weights/ and inputs/ are resolved relative to the repo root
import cv2
import torch
...
def inference(image, background_enhance, face_upsample, upscale, codeformer_fidelity):
    ...
    try: # global try
        ...
        if img is None:
            raise gr.Error('Could not read that file. Use JPG, PNG, WEBP or BMP.')
        ...
        # save restored img
        # unique file per request, so two people using the UI at once never overwrite each other
        fd, save_path = tempfile.mkstemp(suffix='.png', dir='output')
        os.close(fd)
...
demo = gr.Interface(
    ...
    article=article,       
    examples=[
        ['inputs/whole_imgs/00.jpg', True, True, 2, 0.7],
        ['inputs/whole_imgs/01.jpg', True, True, 2, 0.7],
        ['inputs/whole_imgs/03.jpg', True, True, 2, 0.7],
        ['inputs/whole_imgs/04.jpg', True, True, 2, 0.1],
        ['inputs/whole_imgs/05.jpg', True, True, 2, 0.1]
      ]
    )
